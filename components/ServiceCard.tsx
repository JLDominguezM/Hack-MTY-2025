import { Lightbulb, Flame, Droplet, Wifi, Phone, AlertCircle } from 'lucide-react';
import { Card } from './ui/card';
import { Checkbox } from './ui/checkbox';
import { Badge } from './ui/badge';
import type { Service } from './PaymentServicesView';
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div>
              <h3 className="text-gray-900">{service.name} - {service.provider}</h3>
              <p className="text-sm text-gray-500">No. {service.accountNumber}</p>
            </div>
            {isOverdue && (
          ...
          <div className="mt-2 space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className={`text-sm ${
                isOverdue ? 'text-red-600' : daysUntilDue <= 3 ? 'text-orange-600' : 'text-gray-900'
              }`}>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-gray-200">
              ...
              <span className="text-[#EC0000]">
                ${service.amount.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            </div>
          </div>
        </div>
